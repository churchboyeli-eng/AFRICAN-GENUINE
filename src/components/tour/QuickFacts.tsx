        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 divide-y lg:divide-y-0 lg:divide-x divide-line">
          {facts.map((f) => (
            <div key={f.label} className="flex flex-col gap-1 px-6 py-5">
              ...
              <dd className="font-display text-[22px] font-bold tracking-tighter text-cream leading-none mt-1">
                {f.value}
              </dd>
              {f.sub && (
                <span className="font-body text-[11px] text-sand mt-0.5">{f.sub}</span>
              )}
            </div>
          ))}
        </dl>
